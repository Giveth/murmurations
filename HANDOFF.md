...
All secrets live in **`C:\Users\Xerxes\Xerxes-Claude\.secrets\env.json`**
(during the testbed phase). On migration to real hosting, move these
into the host's secret manager (Railway env vars, AWS SecretsManager,
1Password Connect, etc.). Never commit them.
...
The **deployer/minter wallet** for the on-chain contracts has its own
file: **`.secrets/thedaolog_deployer.json`** with `{ privateKey, address }`.
That wallet has `MINTER_ROLE` on the badge contract and is the admin
of the TallyCommit contract. See section 5 for ownership transfer at
handoff.
...
### Handoff: transferring ownership to DevOps

When DevOps takes over, two cleanup actions on Arbiscan:

1. **Replace the BUIDLER minter** — Zep (admin) calls
   `revokeRole(MINTER_ROLE, 0x16D8…42d4)` on the badge contract, then
   `grantRole(MINTER_ROLE, <DevOps wallet>)`.
2. **Replace the TallyCommit admin** — DevOps generates a wallet, gets
   the address, then we call
   `TheDAOLogTallyCommit.transferAdmin(<DevOps wallet>)` from the
   current admin (deployer wallet). Once acknowledged, the deployer
   wallet's powers are gone.

After both, the Xerxes/deployer wallet has zero authority over either
contract and can be safely deleted from `.secrets/`.
...
2. Deploy to **Railway** (or Render, Fly.io) on hobby tier (~$5-10/mo):
   ...
   - Env vars: `PINATA_JWT`, `DEPLOYER_PRIVATE_KEY` (extract from
     `.secrets/thedaolog_deployer.json`).
